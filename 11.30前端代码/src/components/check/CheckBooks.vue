      ...
      <el-table :data="booksList" style="width: 100%" border :header-cell-style="{ background: '#f5f7fa' }">
        ...
        <el-table-column label="操作" align="center" width="120" fixed="right">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-edit" size="mini" @click="showCheckBookDialog(scope.row.book_id)">审核</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页组件 -->
...
export default {
  ...
  methods: {
    ...
    async showCheckBookDialog(bookId) {
      const { data: res } = await this.$http.post("/check/book/findBookById", this.$qs.stringify({ book_id: bookId }));
    ...
    async approveBook() {
      ...
      const { data: res2 } = await this.$http.post("/check/book/approve", { book_id: this.bookInfo.book_id });
