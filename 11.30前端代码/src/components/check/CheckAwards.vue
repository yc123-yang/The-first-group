      ...
      <el-table :data="awardsList" style="width: 100%" border :header-cell-style="{ background: '#f5f7fa' }">
        ...
        <el-table-column label="操作" align="center" width="120" fixed="right">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-edit" size="mini" @click="showCheckAwardDialog(scope.row.award_id)">审核</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页组件 -->
...
export default {
  ...
  methods: {
    ...
    async showCheckAwardDialog(awardId) {
      const { data: res } = await this.$http.post("/check/award/findAwardById", this.$qs.stringify({ award_id: awardId }));
    ...
    async approveAward() {
      ...
      const { data: res2 } = await this.$http.post("/check/award/approve", { award_id: this.awardInfo.award_id });
