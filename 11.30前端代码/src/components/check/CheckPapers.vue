  ...
  <div>
    ...
    <el-table :data="papersList" style="width: 100%" border  :header-cell-style="{ background: '#f5f7fa' }">
      ...
      <el-table-column label="操作" align="center" width="120" fixed="right">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-edit" size="mini" @click="showCheckPaperDialog(scope.row.paper_id)">审核</el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog title="审核论文成果申请" :visible.sync="checkPaperDialogVisible" width="50%">
      <el-form class="dialog" label-width="110px">
        ...
        <el-row>
          <el-col :span="12"
            ><el-form-item label="论文编号：">{{ paperInfo.paper_id }}</el-form-item></el-col
          >
          <el-col :span="12"
...
export default {
  ...
  methods: {
    ...
    async getPaperList() {
      ...
      const { data: res } = await this.$http.post("/check/paper/findAllPaperByCondition", this.$qs.stringify(this.queryInfo));
    ...
    async showCheckPaperDialog(paperId) {
      const { data: res } = await this.$http.post("/check/paper/findPaperById", this.$qs.stringify({ paper_id: paperId }));
    ...
    async approvePaper() {
      ...
      const { data: res2 } = await this.$http.post("/check/paper/approve", { paper_id: this.paperInfo.paper_id });
